...
def initialize_parameters(layers: list) -> dict:
    ...
    for layer in range(1, L):
        parameters["W" + str(layer)] = (
            np.random.randn(layers[layer], layers[layer - 1]) * 0.01
        )
...
def forward(X, parameters, activations):
    ...
    for layer in range(1, L + 1):
        ...
        A, cache = forward_step(A, W, b, act_func)
        caches.append(cache)
...
def predict(X, parameters, activations):
    ...
    """
    AL, _ = forward(X, parameters, activations)
...
def accuracy(X, Y, parameters, activations):
    ...
    """
    preds = predict(X, parameters, activations)
...
def backward_step(dA, cache: tuple, g: str) -> tuple:
    """
...
def backward(AL, Y, caches, activations: list, cost_function) -> dict:
    """
    ...
    if cost_function == "binary_crossentropy" and activations[-1] == "sigmoid":
        dAL = -((Y / AL) - ((1 - Y) / (1 - AL)))
        dZ_L = AL - Y
    elif cost_function == "categorical_crossentropy" and activations[-1] == "softmax":
        dAL = AL - Y
        dZ_L = AL - Y
    ...
    db_L = np.sum(dZ_L, axis=1, keepdims=True) / m  # db_L tendrá dimensión (n_out, 1)
    ...
    for layer in reversed(range(L - 1)):
        ...
        dA, dW, db = backward_step(dA, (A_prev, W, b, Z), activations[layer])
...
def train(X, Y, layers, activations, cost, learning_rate, iterations, print, graph):
    """
