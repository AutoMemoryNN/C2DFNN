...
def backward__pooling_step(
    gradient_in: np.ndarray, specification: Specification_pooling, cache: Cache_conv
...
def backward_convolution_step(gradient_in, specification, cache, parameters):
    ...
    data_in_padded = self.add_pad(cache.data_in, pad)
...
class LayerConv(Layer):
    ...
    def backward(self, gradient_in: np.ndarray, cache: Cache_conv):
        pass
...
class LayerPooling(Layer):
    ...
    def __init__(
        ...
        self.cache = Cache_pooling(np.ndarray([]))
        self.specification = specification

    def forward(self, data_in: np.ndarray):
        data_out, cache = self.forward_pooling_step(data_in, self.specification)
        self.cache = cache
    ...
    def forward_pooling_step(
        self, data_in: np.ndarray, specification: Specification_pooling
    ) -> np.ndarray:
        """
